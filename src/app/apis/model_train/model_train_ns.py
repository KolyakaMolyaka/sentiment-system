...
class ModelTrainWithTeacherAPIv2(Resource):
	...
	@ns.doc(
		security='basicAuth',
		description=\
				'Для обучения модели вместо текстов комментариев пользователь вводит векторы, '
				'полученные любым из способов, и сопоставленные им классы. Размерность каждого вектора '
				'должна быть одинакова. Считается, что каждый элемент вектора представляет степень выраженности '
				'i-го признака признака.'
	)
	def post(self):
